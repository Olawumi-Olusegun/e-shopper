...
const Shipping = ({ addresses }) => {
    ...
    const setShippingAddress = (address) => setShippingInfo(address?._id);
    

    const handleCheckout = async () => {
        if(!shippingInfo) {
            return toast.error("Please set your shipping address")
        }
            ...
            <main className="md:w-2/3">
              <article className="border border-gray-200 bg-white shadow-sm rounded p-4 lg:p-6 mb-5">
                ...
                addresses?.map((address) => (
                  <label key={address?._id} onClick={() => setShippingAddress(address)} className="flex p-3 border border-gray-200 rounded-md bg-gray-50 hover:border-blue-400 hover:bg-blue-50 cursor-pointer">
                    <span>
                      <input
                        name="shipping"
                        type="radio"
                ...
                <div className="flex justify-end space-x-2 mt-10">
                  ...
                  <button onClick={handleCheckout} className="px-5 py-2 inline-block text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 cursor-pointer">
                    Checkout
                  </button>
                </div>
              </article>
              ...
              <article className="text-gray-600" style={{ maxWidth: "350px" }}>
...

                {
                    cart.cartItems?.map((item) => (
                        <figure key={item?._id} className="flex items-center mb-4 leading-5">
                        <div>
                        ...
                        <figcaption className="ml-3">
                            ...
                            <p className="mt-1 text-gray-400">Total: ${item.quantity * item?.price}</p>
                        </figcaption>
                        </figure>
